...
export type ScopedAccessGrant = AccessGrant & {
  policy?: string;
...
function uniqueHosts(grants: ScopedAccessGrant[]): string[] {
...
export function groupStandingApprovals(grants: ScopedAccessGrant[]): StandingGroup[] {
  const order: string[] = [];
  const byItem = new Map<string, ScopedAccessGrant[]>();
  for (const g of grants) {
...
function standingList(standing: ScopedAccessGrant[]): SafeHtml {
...
export function agentRow(c: AccessClient, environments: string[], standing: ScopedAccessGrant[]): SafeHtml {
