...
export const MCP_INSTRUCTIONS_HOSTED = [
  ...
  "Example: user says get my Spotify profile → http_request method GET path https://api.spotify.com/v1/me. Host can be api.spotify.com with path /v1/me. A full https path URL is enough.",
...
export const HOSTED_TOOL_PARAM_DESCRIPTIONS = {
  ...
  http_body:
    "Object body for POST, PUT, or PATCH. JSON by default. For Spotify /api/token use content_type application/x-www-form-urlencoded (or omit it; Botpasses form-encodes that path). Omit for GET.",
  http_content_type:
