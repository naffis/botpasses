...
- Repository: vendored agent-harness scaffolding (`.cursor/*`, `.loadout/`, `processes/`, loadout docs) removed; `INSTALL.md` now describes Botpasses. Migrations `006_identity_mfa.sql` and `007_oauth_tenancy_and_payload_columns.sql` mirror the schema changes (007 needs a duplicate check on existing databases; see the file header).
