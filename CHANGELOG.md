...
- Infra: the site builds on Astro 7 (`npm --prefix site audit` was failing on advisories fixed only in 7.1+); `@astrojs/markdown-remark` is installed for the docs table rehype plugin. The `secrets-scan` CI job has `pull-requests: read` so gitleaks can list the PR's commits. `npm audit` in CI and the prod deploy runs through `scripts/audit.sh` (45 s per attempt, four attempts) because the registry's advisory endpoints sometimes hang; in CI the audits are their own `audit` job so they cannot consume the `test` job's time budget.
