...
- Infra: the site builds on Astro 7 (`npm --prefix site audit` was failing on advisories fixed only in 7.1+); `@astrojs/markdown-remark` is installed for the docs table rehype plugin. The `secrets-scan` CI job has `pull-requests: read` so gitleaks can list the PR's commits. `npm audit` in CI and the prod deploy runs through `scripts/audit.sh`, which bounds the registry fetch and retries, because the bulk advisory endpoint sometimes hangs and its fallback rejects valid lockfiles.
