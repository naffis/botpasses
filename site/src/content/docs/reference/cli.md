...
| `VAULT_PUBLIC_URL` | Hosted origin for `vault login` and `vault mcp --user-jwt`. First-party botpasses.com, a self-host `https` origin, or loopback on plane `dev` |
...
`npm run hosted:dev` starts the same kernel on loopback with sqlite-hosted (or `--postgres`). It writes `.botpasses-hosted/` and prints OTP codes in the terminal. Run one process per sqlite file (a second writer can get SQLITE_BUSY). The CLI vault callback stays `http://127.0.0.1:8888/callback`; hosted connect uses `{origin}/connect/callback`.
