...
Run `npx vault` from the cloned repository (or `npm run botpasses -- <command>`) until the npm package is published. Process environment names are `VAULT_*`. The local server listens on **8788**. `vault <command> --help` prints the usage for every command.
