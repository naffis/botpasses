...
| GET | `/robots.txt` | Production disallows console, auth, collect, api, mcp, oauth. Lists `Sitemap: https://botpasses.com/sitemap.xml`. Comments point at `/llms.txt` and `/llms-full.txt` |
...
| POST | `/api/integrations/:provider/start` | `:provider` is `spotify`, `github`, `google`, `slack`, or `stripe` (unknown is 404). `{ "item_name", "environment?", "client_id?", "redirect_uri?", "agent_client_id?", "need_id?" }`. Returns `authorize_url`, `redirect_uri`, and `provider` for a user connect. `agent_client_id` names one agent that gets a standing approval on the source credential and the refresh item after connect; `need_id` closes the Inbox request the connect answers |
