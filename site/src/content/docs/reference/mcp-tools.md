...
Connector rules: exact `allowed_hosts` match, no IP literals, DNS resolved and pinned to public addresses, no redirects followed, 10 s timeout, the credential and any minted access tokens replaced with `[redacted]` in the body. Responses over 1 MiB are cut at the socket and returned as `origin_status: 502` with `{ "error": "body_too_large" }`; ask the API for a smaller page. The model sees at most 256 KiB of text.
