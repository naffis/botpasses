...
Unauthenticated `initialize`, `ping`, `tools/list`, and `notifications/*` succeed, so a client with a preconfigured bearer does not get a connect card. Unauthenticated `tools/call` and `GET /mcp` (the SSE stream) are 401 with `WWW-Authenticate` carrying `resource_metadata` for `/.well-known/oauth-protected-resource/mcp`, so a Streamable HTTP host can start OAuth. Trusted runtime tokens (`avt_...`) cannot call MCP.
