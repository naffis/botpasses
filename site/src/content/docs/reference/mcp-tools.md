...
Unauthenticated `POST /mcp` (including `initialize`, `ping`, `tools/list`, and `tools/call`) and `GET /mcp` (the SSE stream) are 401 with `WWW-Authenticate` carrying `resource_metadata` for `/.well-known/oauth-protected-resource/mcp` and `scope="mcp"`. Grok Bot builds the Authorize URL from a 401 on `initialize`. A preconfigured `avm_...` Bearer is enough for every method and skips the connect card. Trusted runtime tokens (`avt_...`) cannot call MCP.
...
| `status: "user_connect_required"`, `provider`, `item_name`, `refresh_item_name`, `connect_url`, `need_id`, `hint` | A path that answers only for a signed-in user (Spotify `/v1/me`) was called with the app credential and no connected account. Nothing was sent and no approval was spent. The operator opens `connect_url` (a console link that also shows the request in the Inbox), connects their account, and allows the agent to use it; the agent then retries the same call once. Not an MCP error |
...
A `prompt` approval is one call: any answer from the API spends it, including 401, 410, or a 5xx. It comes back only when the request never left Botpasses (a host mismatch, `user_connect_required`, or a DNS, connect, or TLS failure). When the agent needs to retry, approve with limits (max calls or a duration) instead.
