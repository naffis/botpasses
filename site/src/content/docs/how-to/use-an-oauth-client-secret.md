...
2. Add the redirect URI the console shows. On production it is `https://botpasses.com/connect/callback`, on staging `https://staging.botpasses.com/connect/callback`, and locally `http://127.0.0.1:8888/callback`. Same URI for every provider. Copy it from Store credential (Kind: Client ID and secret) or from Connect. Older apps that already registered `/integrations/<provider>/callback` still work.
