...
2. Add the redirect URI the console shows. On production it is `https://botpasses.com/connect/callback`, on staging `https://staging.botpasses.com/connect/callback`, and on `npm run hosted:dev` it is `http://127.0.0.1:8788/connect/callback`. The local CLI vault (`vault serve`) still uses `http://127.0.0.1:8888/callback`. Same URI for every provider on a given plane. Copy it from Store credential (Kind: Client ID and secret) or from Connect. Older apps that already registered `/integrations/<provider>/callback` still work.
...
4. Tell the agent you are done. It retries the same call once and gets the user's data. Later calls to that API with **Also allow** left checked do not open another Inbox card. The standing approval is on `SPOTIFY_SECRET` (the API), not only on the token mint.
