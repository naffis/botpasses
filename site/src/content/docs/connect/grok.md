...
After the connector is on, paste a [hosted bootstrap prompt](/docs/prompts#hosted) for help storing credentials and connecting API accounts. Enter secrets on the Botpasses site you connected to. Do not paste them into chat. If `tools/call` fails with `redirect_uri`, paste the [Grok redirect_uri prompt](/docs/prompts#grok-redirect_uri). Grok Bot in a cloud VM cannot reach `npm run hosted:dev` on your laptop; use botpasses.com or a public self-host origin.
