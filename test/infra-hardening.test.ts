...
test("N7: CF-Connecting-IP is read only when the peer Fly saw is inside the trusted proxy ranges", () => {
  const trust = { trustFlyHeader: true, trustedProxyCidrs: CLOUDFLARE_PROXY_CIDRS };
  ...
  // Fly header not trusted (outside Fly): the last X-Forwarded-For hop is the peer.
  assert.equal(
    clientIpFromHeaders(
      { cfConnectingIp: "198.51.100.7", flyClientIp: "104.16.1.1", forwarded: "10.0.0.1, 203.0.113.9" },
      { ...trust, trustFlyHeader: false },
    ),
    "203.0.113.9",
  );
