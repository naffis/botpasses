...
Product name, `VAULT_*` env prefix, Fly app names, and Clerk `authorizedParties` omission in 0001 still hold.
...
Clerk FAPI hosts are `clerk.botpasses.com` and `clerk.staging.botpasses.com`. Resend domain is `mail.botpasses.com`. Documented From is `Botpasses <noreply@mail.botpasses.com>` via `VAULT_EMAIL_FROM`.
...
- Fly secrets `VAULT_PUBLIC_URL`, `VAULT_EMAIL_FROM`, and `CLERK_FRONTEND_API` must use `.com` (not `.ai`).
- `fly.staging.toml` / `fly.prod.toml` set `VAULT_PUBLIC_URL` to the plane origin so a missing secret cannot emit another hostname.
- Cloudflare DNS, Fly certs, Clerk FAPI CNAMEs, and Resend records are on the `.com` zone. DNS is A/AAAA to Fly IPs, not a user-facing hostname on the platform default domain.
