...
Amended 2026-09-10 (BOTP-13): a 401 on GET or HEAD `/mcp` (the SSE listen) sends the same Bearer `WWW-Authenticate` with `resource_metadata` as `POST /mcp` `tools/call`. Streamable HTTP hosts open that stream during connect; omitting the challenge left needsAuth with no auth URL (Authorize → Retry, no browser). A rejected DCR `redirect_uri` logs `dcr_redirect_rejected` with scheme and reason only.

Amended 2026-09-10 (BOTP-13 follow-up): unauthenticated `POST /mcp` handshake methods (`initialize`, `ping`, `tools/list`, `notifications/*`) are also 401 with that challenge plus `scope="mcp"`. Grok Bot's AuthenticateMcpServer builds `auth_link` from a 401 on `initialize`. An anonymous 200 there yielded `no_auth_link` even when GET `/mcp` already advertised PRM. A preconfigured `avm_` Bearer still skips the card because the host sends the header.
