# 0003. Grant-vault, not zero-knowledge
...
Local CLI stays closer to operator-held keys (`VAULT_MASTER_KEY` / `master.key`). Hosted comfort comes from no-reveal, KMS wrap of the platform KEK ([0004](0004-kms-wrapped-kek.md)), audit, and isolation tests that fail if a canary appears in the model.
