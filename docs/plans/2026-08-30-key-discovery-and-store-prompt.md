...
- **A-5.** `collect_url` is `{VAULT_PUBLIC_URL}/collect/{needId}` with no query string. Staging origin may be `https://botpasses-staging.fly.dev` until `staging.botpasses.ai` resolves.
