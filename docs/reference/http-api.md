...
JSON bodies are capped at 128 KiB and must be sent as `Content-Type: application/json` (any other type is `415 { error: "Content-Type must be application/json" }`; a POST with no body and no type reads as `{}`). Only `POST /approve` and `POST /consent`, which back HTML forms, also accept `application/x-www-form-urlencoded`. Hosted operator mutations need a session cookie plus `X-CSRF-Token`. Invalid Bearer on public HTML is ignored (200). Invalid Bearer on `/api` and every `/mcp` method (including `initialize`) is 401 with `WWW-Authenticate` `resource_metadata` and `scope="mcp"`. A valid `avm_…` is sufficient for all MCP methods and skips the connect card.
...
| GET | `/robots.txt` | none | Staging allows `/`. Prod disallows `/console`, auth, collect, api, mcp, oauth. Prod lists `Sitemap: https://botpasses.com/sitemap.xml` and comments pointing at `/llms.txt` and `/llms-full.txt` |
| GET | `/llms.txt`, `/llms-full.txt`, `/.well-known/security.txt`, `/security.txt`, `/favicon.ico`, `/apple-touch-icon.png`, `/favicon.svg`, `/mark.svg`, `/mark-on-dark.svg`, `/og.png`, `/og-square.png`, `/logo.png`, `/android-chrome-192x192.png`, `/android-chrome-512x512.png`, `/site.webmanifest` | none | Static from `site/dist`. `llms.txt` is the curated index; `llms-full.txt` is every docs page in one file. `/security.txt` is the same bytes as the well-known file. `/mark.svg` is the navy ticket; `/mark-on-dark.svg` is the white ticket. `/logo.png` is the square favicon raster for JSON-LD; `/og.png` is the 1200x630 social card |
...
| POST | `/api/integrations/:provider/start` | operator | `:provider` is a registry id with a user connect flow (`spotify`, `github`, `google`, `slack`, `stripe`; unknown is 404). `{ item_name, environment?, client_id?, redirect_uri?, agent_client_id?, need_id? }` → `{ authorize_url, redirect_uri, provider }`. `client_id` defaults to the item username. `agent_client_id` names the one model client that gets an `item_standing` policy on the source credential and the refresh item after connect; without it the operator approves each API call normally. `need_id` names the pending `connect` inbox need this flow answers (404 unless it is this org's, pending, and for `item_name`); the callback marks it fulfilled. The sealed `state` carries the provider id, the agent, and the need. |
| GET | `/connect/callback` | operator cookie (ready) | Exchanges the code with the client-secret item, stores the refresh token as `<ITEM>_REFRESH` (`inject: refresh`, allowed on the provider API and token hosts), redirects to `/console#vault?connected=<provider>` (plus `&agent=<clientId>` when an agent was named, so the landing flash says it can retry) or `?connect_error=<provider>&reason=<code>` where `reason` is `state_expired` (invalid, expired, or foreign state), `provider_denied` (the provider sent `error`), `exchange_failed` (non-2xx code exchange), or `no_refresh_token`. The provider id is in the sealed `state`. A state minted for another provider, another org, or another operator account is refused. An existing `<ITEM>_REFRESH` is rotated and its hosts and inject mode reset to the provider defaults. Audit: `provider_connected`, plus `grant` when `agent_client_id` was given, plus `need_fulfilled` when `need_id` was given (the need's `item_id` is the refresh item). `/integrations/:provider/callback` remains as an expand-only alias for apps that already registered that URI. `/api/integrations/:provider/start` still names the registry provider. A user-path `http_request` on `<ITEM>` reads `<ITEM>_REFRESH` for the mint without a second grant; posting the refresh item to the token endpoint directly still needs an approval on `<ITEM>_REFRESH`. Botpasses adds the client secret from `<ITEM>` per the provider's token auth (see [mcp.md](mcp.md), Providers). |
...
| GET | `/api/inbox` | | `{ grants, needs }` pending. Each need carries `kind` (`secret` or `connect`), `suggested_name`, `host`, `client_id`, `client_name` (the agent), `task_description`, `collect_path` (`secret` only, else null), `provider`, `source_item_id`, `source_item_name` (`connect` only, else null), `created_at`, `expires_at`, `status`. A `connect` need is "<agent> needs a <provider> account for <item>": its card opens the item's connect dialog at `#credentials/item/<source_item_id>?connect=<provider>&agent=<client_id>&need=<id>` |
| GET | `/api/audit` | optional `?client_id=` and `?item_name=` | `{ audit }` actions, item names, client ids, and `host` on `auto_approved`. Never values |
| GET | `/api/need-items/:id` | operator + same org | need metadata including `kind`, `provider`, and `recipe` when the host matches a setup recipe (`allowed_hosts`, `kind`, `inject`, `username_required`, `hint`, `dashboard_url`, `connect_after`, `primary_host`). Not public JSON; unsigned or another org's need is **404**, never 403 |
| POST | `/api/need-items/:id/fulfill` | `{ value, name?, allowed_hosts?, inject?, kind?, username?, always_allow? }` | `{ item, grant_status }`. `always_allow: true` (JSON boolean only) writes an `item_standing` policy and standing grant in the same transaction as the item. Omitted or not `true` stays a one-call prompt grant. A `connect` need is 409 (nothing is typed in; `GET /collect/:id` for one is a 302 to `/console#inbox`) |
| POST | `/api/need-items/:id/deny` | operator + same org | `{ ok: true }`. Status `denied`, audit `need_denied`; 409 when not pending; another org's need is 404 |
...
| POST | `/api/grants/request` | model or operator | `{ item_name, task_description?, client_id?, operator_email? }`. `operator_email` is ignored for model principals; an operator may pass a member's email (400 otherwise); with none given every verified member is emailed. Optional `host`, `method`, `path` (the call the agent will make; see MCP `request_grant`). Returns the existing open grant when it covers that call (or when no call was stated); a standing grant that does not cover host/method/path yields a pending grant for the requested scope (a pending one gets a fresh `approval_code`). Rate limit 30 / org / hour inside the kernel, shared with `http_request` and new needs |
| POST | `/api/grants/:id/approve` | operator | `{ policy, confirm_name?, scope? }`. `scope: { methods?, path_prefixes?, hosts?, max_calls?, ttl_seconds? }`; omit to inherit the requested call (or no limits); present dimensions are used exactly; `hosts` must be a subset of the item's hosts (400). `ttl_seconds` 60..86400 for `session` (default 28800), 60..31536000 for standing policies, which then expire. `folder_standing` is owner + confirm. A grant for a trusted (`avt_`) client refuses an explicit `methods`, `path_prefixes`, or `hosts` with 400 `scope_unenforceable`: the runtime resolves the value directly and nothing checks those limits. A plain approve (no `scope`) does not inherit the requested call for such a client and is written unrestricted; `max_calls` / `ttl_seconds` are counted on resolve and stay allowed. A standing approval replaces a stale policy row for the same pair |
...
`site/dist` (Astro) is served by [src/hosted/static-site.ts](../../src/hosted/static-site.ts) for `/`, `/design`, `/security`, `/privacy`, `/terms`, `/changelog`, `/docs/**`, `/_astro/**`, `/pagefind/**`, `/sitemap.xml`, `/sitemap-*.xml`, `/favicon.svg`, `/mark.svg`, `/mark-on-dark.svg`, `/favicon.ico`, `/apple-touch-icon.png`, `/og.png`, `/og-square.png`, `/logo.png`, `/android-chrome-192x192.png`, `/android-chrome-512x512.png`, `/site.webmanifest`, `/llms.txt`, `/llms-full.txt`, `/.well-known/security.txt`, and `/security.txt`. `*.html` and trailing-slash forms 308 to the canonical URL (`/console/` is handled by the router, not here). Unknown paths under those prefixes return the Astro `404.html` with status 404. HTML is `no-cache` from the static layer (the router's `no-store` wins today), `_astro/*` is `public, max-age=31536000, immutable`, other assets `public, max-age=3600`. The site, like every page, is served only when the `Host` header is the public origin (`403 { error: "Origin/Host not allowed" }` otherwise); `/health`, `/ready`, discovery, and `/assets/*` answer on any Host for platform checks. A foreign browser `Origin` on those public GET/HEAD/OPTIONS paths is allowed and reflected in `Access-Control-Allow-Origin`; it does not bypass the Host check.

First-party assets (`console.js`, `console.css`, `auth.js`, `auth.css`, `collect.js`, `mark.svg`, `mark-on-dark.svg`) are referenced by the pages at content-hashed paths, `/assets/console.<sha8>.js`, and served `public, max-age=31536000, immutable`; the hash changes with the body, so a deploy never fights a cached bundle. The plain names still resolve to the current body but are `no-cache`.
...
Inbox grant cards (`GET /api/inbox`) carry `requested_scope`, `grant_scope`, `allowed_hosts`, `expires_at`; `GET /api/access` grants carry `item_id` (the exact credential, independent of its name or environment), `policy`, `expires_at`, `grant_scope`. Audit actions: `scope_denied`, `grant_exhausted`, `auto_approved` (a standing `item_standing` or `folder_standing` approval covered `http_request` or resolve; row includes item name, client, and host, never values), `client_environment`, `client_reactivated`, `inject_denied` (the connector refused before dialing), `inject_failed` (the origin was unreachable before the TLS handshake, so the value never left), `inject` (the value left the process, whatever came back: a status, a token endpoint's unusable 2xx body, a close mid-response, or a deadline after the handshake), `refresh_rotated` (a provider rotated an `<ITEM>_REFRESH` value; actor `provider`), `connect_requested` (an agent hit a user-only provider path with only the app credential; a `connect` need was created), `need_denied` (the operator denied a need from the inbox), `dek_rewrapped` (actor `system`; an org DEK moved from the previous KEK to the current one during a rotation). `allowed_hosts` are stored trimmed, lowercase, and deduplicated on create and update.
...
| 403 | CSRF, MFA, wrong channel, foreign Origin on `/api`, `/console`, `/collect`, and auth (no ACAO). `/mcp` and `/oauth` reflect a foreign Origin. Public site GET/HEAD/OPTIONS (`/`, `/docs`, `/og.png`, `/logo.png`, `/favicon.ico`, …) also reflect a foreign Origin so unfurlers can load cards and icons. |
