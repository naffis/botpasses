...
| GET | `/robots.txt` | none | Staging allows `/`. Prod disallows `/console`, auth, collect, api, mcp, oauth. Prod lists `Sitemap: https://botpasses.com/sitemap-index.xml` and comments pointing at `/llms.txt` and `/llms-full.txt` |
| GET | `/llms.txt`, `/llms-full.txt`, `/.well-known/security.txt` | none | Static from `site/dist`. `llms.txt` is the curated index; `llms-full.txt` is every docs page in one file |
