...
| GET | `/llms.txt`, `/llms-full.txt`, `/.well-known/security.txt`, `/security.txt`, `/favicon.ico`, `/apple-touch-icon.png` | none | Static from `site/dist`. `llms.txt` is the curated index; `llms-full.txt` is every docs page in one file. `/security.txt` is the same bytes as the well-known file |
