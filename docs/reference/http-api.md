...
| GET | `/api/need-items/:id` | operator + same org | need metadata including `kind` and `provider` (not public JSON; unsigned or another org's need is **404**, never 403) |
| POST | `/api/need-items/:id/fulfill` | `{ value, name?, allowed_hosts?, inject?, kind?, username? }` | `{ item, grant_status }`. A `connect` need is 409 (nothing is typed in; `GET /collect/:id` for one is a 302 to `/console#inbox`) |
