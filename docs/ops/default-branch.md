...
| `production` | `FLY_API_TOKEN` (Fly deploy token scoped to `botpasses-prod`) | Deployment branches: `dev` only. Required reviewers need a GitHub plan that allows environment reviewers on a private repo (this personal private repo rejected that rule with HTTP 422). | `deploy-prod.yml` |
...
1. Settings, Environments: create `staging`, `production`, and `backup` if they are missing. On each, set "Deployment branches and tags" to selected branches: `dev`. On `production`, add at least one required reviewer if the GitHub plan allows it (`deploy-prod.yml` waits on that review). A personal private repo on a plan without environment reviewers cannot set that rule.
...
| --- | --- | --- | --- |
| 2026-09-05 | `botpasses-20260905T135436Z.dump.enc` (1633 bytes; `backup-verify` green on [run 33970231210](https://github.com/naffis/botpasses/actions/runs/33970231210)) | Neon `botpasses-prod` (`falling-grass-67957321`) scratch branch `restore-drill-20260905`. Decrypt produced a `PGDMP` archive. `pg_restore --no-owner --clean --if-exists` applied it; two ignored errors were Neon `cloud_admin` default-privilege ALTERs, not user tables. The dump is the first offsite copy of the isolated prod project (empty schema). | agent |
