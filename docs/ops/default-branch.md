...
| --- | --- | --- | --- |
| 2026-09-05 | `botpasses-20260905T135436Z.dump.enc` (1633 bytes; `backup-verify` green on [run 33970231210](https://github.com/naffis/botpasses/actions/runs/33970231210)) | Neon `botpasses-prod` (``) scratch branch `restore-drill-20260905`. Decrypt produced a `PGDMP` archive. `pg_restore --no-owner --clean --if-exists` applied it; two ignored errors were Neon `cloud_admin` default-privilege ALTERs, not user tables. The dump is the first offsite copy of the isolated prod project (empty schema). | agent |
