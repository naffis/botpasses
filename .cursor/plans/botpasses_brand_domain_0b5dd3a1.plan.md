...
todos:
  - id: t01-brand
    ...
    status: completed
  - id: t02-surfaces
    content: T-02 Wire UI, MCP, health, CLI, home, boot VAULT_EMAIL_FROM, email From fail-closed
    status: completed
  - id: t03-package
    content: T-03 Rename package to botpasses; dual bin botpasses + vault; refresh lockfile
    status: completed
  - id: t04-infra-config
    content: T-04 Rename Fly apps in tomls, .env.example, backup prefix (do not push until T-07 apps exist)
    status: completed
  - id: t05-docs
    content: T-05 Update README, AGENTS.md, CHANGELOG 0.3.0, ADR 0001, ops cutover runbook
    status: completed
  - id: t06-tests-gate
    content: T-06 Finish AC tests including boot email and AgentPass unchanged; npm test && typecheck
    status: completed
  - id: t07-operator
    content: T-07 Create Fly apps, copy secrets, DNS/certs/Clerk/Resend, then push; GitHub rename last
    status: completed
isProject: false
---
